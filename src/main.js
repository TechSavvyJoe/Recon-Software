    ...
    for (let i = 1; i < lines.length; i++) {
      if (lines[i].trim()) {
        ...
        headers.forEach((header, index) => {
          vehicle[header] = values[index] ? values[index].replace(/["\r\n]/g, '').trim() : '';
        });
        
        if (vehicle['Stock #'] || vehicle['Stock#']) {
          const processedVehicle = {
            ...
            'Status': 'New Arrival',
            'Detailer': '',
            'Date In': vehicle['Inventory Date'] || vehicle['Created'] || new Date().toISOString().split('T')[0],
            'Notes': vehicle['Tags'] ? `Tags: ${vehicle['Tags']}` : '',
            'Last Updated': new Date().toISOString(),
